...
describe('NurseryCreator コンポーネント', () => {
  ...
  describe('バリデーション', () => {
    ...
    test('不正な日付形式が内部的に検出された場合はエラーメッセージが表示される', async () => {
      // このテストは、バリデーション関数が不正な日付文字列を正しく検出することを確認する
      const user = userEvent.setup();
      renderWithProviders(<NurseryCreator onCancel={vi.fn()} />);
      ...
      // バリデーション関数を直接テストするため、
      // コンポーネントの内部状態に不正な日付を設定する
      // 実際のユーザー操作では、HTML5 date inputにより制限されるが、
      // バリデーション機能自体が正しく動作することを確認する
