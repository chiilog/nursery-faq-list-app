...
```
src/components/
├── ui/                    # Chakra UI v3 生成ファイルのみ
├── layout/                # レイアウト・ナビゲーション関連
├── routing/               # ルーティング・分析関連
├── features/              # 機能別コンポーネント
│   ├── nursery/          # 保育園管理機能
│   ├── nursery-creator/  # 保育園作成機能
│   ├── questions/        # 質問機能
│   ├── insights/         # インサイト機能
│   └── cookie-consent/   # Cookie同意機能
└── shared/               # 共通コンポーネント
```

### ディレクトリ責務の定義

#### `ui/` - Chakra UI v3 生成ファイル

- Chakra UI v3のCLIツールから生成されたコンポーネントのみ
- 手動で編集しない、カスタムコンポーネントは配置しない

#### `layout/` - レイアウト・ナビゲーション

- アプリケーション全体のレイアウト構造
- ナビゲーション要素（ドロワー、ボトムナビゲーション）
- スクロール制御などのレイアウト関連機能

#### `routing/` - ルーティング・分析

- React Routerのルーティング設定
- ページ遷移時の分析イベント送信
- ルート保護やリダイレクト処理

#### `features/` - 機能別コンポーネント

- **nursery/** - 保育園の表示・管理に関する機能
- **nursery-creator/** - 保育園の新規作成・編集機能
- **questions/** - 質問の追加・編集・表示機能
- **insights/** - 気づきタグの管理機能
- **cookie-consent/** - Cookie同意・プライバシー機能

#### `shared/` - 共通コンポーネント

- 複数の機能で使用される汎用コンポーネント
- 特定の機能に依存しないUI要素
- フォーム部品や表示コンポーネント
